---
title: "Validation Part 1: Hardware Pre-Validation via Live Boot"
description: "Before installing OKD, prove every component works. CentOS Stream 10 live boot, three storage tiers, Mellanox CX4121C at PCIe 3.0 x8, and 9.41 Gbps through the switch."
date: 2026-03-25
...
You don't install a platform on hardware you haven't tested. That sounds obvious, but the homelab community is full of "I just installed Proxmox and it worked" posts that skip the part where you verify that your NIC actually negotiates at the right PCIe width, your boot SSD isn't already worn out, and your three storage tiers can sustain concurrent I/O without bus conflicts.

This post covers Stage -1: hardware pre-validation using a CentOS Stream 10 live boot on the Dell OptiPlex 5090 SFF. The goal is a repeatable, node-agnostic checklist that proves every component before OKD touches the disk.
...
Everything needed for the full checklist installs in under a minute. The live environment runs entirely from RAM — nothing touches the disks until you explicitly tell it to.
...
PASSED. Zero reallocated sectors. 34,600 power-on hours — that's about 4 years of continuous operation, typical for a decommissioned enterprise drive. The Media_Wearout_Indicator at 0 with normalized 100/100 confirms minimal MLC wear. This drive has plenty of life left for etcd.
...
Individual drive tests don't catch bus sharing conflicts. The real question: can all three drives sustain simultaneous reads through the PCH without errors?
...
No bus conflicts, no I/O errors in `dmesg`. The Q570 chipset handles all three tiers without contention.

## The NIC journey: three cards, three problems

### Intel X710-DA2 — POST hang

The Intel X710-DA2 was the first choice — dual SFP+ 10GbE, `i40e` driver, and [Red Hat Ecosystem Catalog certified](https://catalog.redhat.com/en/hardware/detail/236147). For a platform running SCOS (same kernel as RHEL), a Red Hat certified NIC seemed like the safe bet.

It wasn't.

Installed the X710 in the PCIe x16 slot, connected a DAC cable to the CRS317, powered on. The system hung at POST — frozen on the Dell splash screen. No BIOS, no boot menu, nothing. JetKVM showed a static Dell logo indefinitely.

**Attempt 1: Disable Option ROM.** The X710 has a PXE/UEFI Option ROM that executes during POST. Pulled the NIC, booted into BIOS, disabled Option ROM for the PCIe slot, shut down, reinstalled. Same hang.

**Attempt 2: Every BIOS combination.** Disabled PXE boot, toggled UEFI network stack, changed PCIe slot configuration. Nothing. With the X710 installed, the system wouldn't POST. Without it, perfectly fine.

**Attempt 3: Reseat.** Only one usable PCIe slot in the 5090 SFF. Reseated the card, confirmed full engagement. Same result.

Searching turned up scattered forum reports of Intel X710 + Dell OptiPlex compatibility issues — PCIe lane negotiation conflicts with the Dell BIOS/firmware. No official Dell or Intel documentation acknowledges it. Returned the X710.

### Mellanox ConnectX-3 CX312A — driver dropped from SCOS 10

Second attempt: Mellanox ConnectX-3 CX312A. Dual SFP+ 10GbE, `mlx4_core` driver. Cheaper and widely available used.

I first tested it by booting an RHCOS live image (from an earlier OpenShift evaluation). PCIe detected fine, both ports visible, link up. Looked good.

Then I switched the project from OCP to OKD. OKD 4.20 runs SCOS 10, not RHCOS. Booted CentOS Stream 10 live (same kernel as SCOS 10) — `lspci` showed the card, PCIe negotiation was fine. But `ip link` showed no Mellanox interfaces. No `mlx4_core` module loaded.
...
Third attempt: Mellanox ConnectX-4 Lx CX4121C (Dell P/N 20NJD). Dual SFP28 25GbE, `mlx5_core` driver. Dell OEM card — no chassis compatibility risk. `mlx5_core` is the current Mellanox driver, actively maintained, included in every modern kernel.

Installed it. System booted normally. Both ports detected. Everything worked.
...
The kernel dmesg confirms both ports came up during boot:
...
Full width. Speed 8GT/s (PCIe 3.0), Width x8, no "(downgraded)" warnings. That's ~8 GB/s of PCIe bandwidth — more than enough for dual 25 GbE (which maxes out at ~6.25 GB/s aggregate).

<Callout title="If you see downgraded width" variant="warning">
  If LnkSta shows x4 or x1, reseat the card. The SFF riser's contact pressure can be marginal. PCIe 3.0 x4 would still handle 10 GbE but would bottleneck Ceph under heavy replication load.
</Callout>
...
MikroTik XS+DA0001: 25G Base-CR CA-N, 1 meter passive copper DAC. At this length, no signal integrity concerns. Port 2 shows identical results:
...
Firmware 14.32.2004 — the Dell OEM build. Current, no upgrade needed. The Dell part number (0Z0NJD, sold as Dell 20NJD) confirms this is the Dell-branded CX4121C.

## Onboard NIC and basic connectivity

The Intel onboard NIC (`enp0s31f6`) handles management traffic and OKD cluster networking:
...
2 x 32 GB DDR4-3200 in DIMM1 and DIMM2 — dual channel. DIMM3 and DIMM4 are empty, leaving room to double to 128 GB if needed.
...
The `vmx` flag is present in `/proc/cpuinfo`, and DMAR/IOMMU is fully initialized:
...
The final validation: can the Mellanox NIC push line-rate traffic through the CRS317 switch? This isn't a loopback test — it's a real path through the switch fabric.

The Mac mini is connected to CRS317 sfp-sfpplus12 (10 GbE RJ45). Node 4 is on sfp-sfpplus2 (10 GbE DAC). Both ports are in the switch's bridge. iperf2 with 8 parallel TCP streams, 30 seconds:
...
9.41 Gbps. That's 94% of the 10 GbE theoretical maximum — essentially line rate after TCP overhead. The CRS317's Marvell Prestera ASIC is forwarding at hardware speed.

This is the path Ceph replication traffic will take in Phase 1. It works.
...
Record these. When SCOS 10 ships a different kernel version, you'll want to compare driver behavior.
...
With a GO from Stage -1, the hardware is proven. The next post covers Stage 0: deploying OKD 4.20 as a Single Node OpenShift cluster on this validated hardware — including every DNS pitfall, installer gotcha, and platform-specific workaround that the official docs don't mention.
