...
By adding ports to `bridge-lan`, you create a single switching domain. Frames from any port in the bridge can reach any other port in the bridge — like plugging everything into the same unmanaged switch. Then VLAN filtering on top of that bridge creates the segmentation: ports tagged for VLAN 5 can only talk to other VLAN 5 ports, VLAN 10 only to VLAN 10, etc.
