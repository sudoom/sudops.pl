...
The [network design post](/blog/homelab-design/network) defined VLANs, firewall policy, and device requirements. This post is where that design hits real hardware — a MikroTik CCR2004 router and CRS317 switch, configured from factory reset to a working VLAN-segmented network.

This is the minimal viable configuration for Phase 0: one node, two VLANs, two devices. Everything else waits.
...
The fix: don't use VLAN 1. We picked VLAN 5. Set it as PVID on all access ports, carry it tagged on the trunk. Everything works immediately.
...
In MikroTik, a bridge is a virtual switch — it connects physical ports at L2 so frames can flow between them. Without a bridge, each port on the CCR2004 is an isolated interface. Devices on ether1 can't talk to devices on ether8. Nothing works.

By adding ports to `bridge-lan`, we create a single switching domain. Frames from any port in the bridge can reach any other port in the bridge — like plugging everything into the same unmanaged switch. Then VLAN filtering on top of that bridge creates the segmentation: ports tagged for VLAN 5 can only talk to other VLAN 5 ports, VLAN 10 only to VLAN 10, etc.
...
<Callout title="vlan-filtering=no is deliberate" variant="warning">
  We create the bridge with filtering off and enable it as the very last step, after everything else is verified. Enabling it with incorrect VLAN table entries will silently drop frames and lock you out. Every MikroTik VLAN guide warns about this. Every homelab builder learns it the hard way.
</Callout>
...
The CCR2004 sits between the private network (192.168.x.x) and the public internet. Devices on the LAN use RFC1918 addresses — these aren't routable on the internet. NAT (masquerade) rewrites the source address of outgoing packets to the router's public IP, and rewrites it back on the return path. Without it, no device on the network can reach the internet.
...
The WAN interface has a public IP (ISP in bridge mode). Packets arriving from the internet with private source addresses (192.168.x.x, 10.x.x.x, 172.16-31.x.x) are spoofed — they shouldn't exist on a public link. Drop them before they reach the firewall:
