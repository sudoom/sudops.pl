...
```yaml
# bootstrap/phase0/01_catalog_source.yaml
...
```yaml
# bootstrap/phase0/02_gitops_operator.yaml
apiVersion: operators.coreos.com/v1alpha1
kind: Subscription
metadata:
...
```yaml
# bootstrap/phase0/03_cluster_admin_rbac.yaml
...
```yaml
# bootstrap/root-app/templates/applications.yaml
...
This is the one Application I apply by hand — it points ArgoCD at the `bootstrap/root-app` chart, which renders everything else:

```yaml
# bootstrap/phase0/05_root_application.yaml
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: root-app
...
Next: [cert-manager](/blog/homelab-day2/cert-manager) — the first component deployed entirely through this pipeline.
