...
```yaml
# components/operators/cert-manager/templates/operator.yaml
...
cert-manager needs a Cloudflare API token to create DNS-01 challenge TXT records. This secret can't live in Git as plaintext (it's a credential), so it's the one manual step outside the pipeline:
...
*Later I fold this into a SealedSecret so even the token lives in Git — for now, it's the one manual step.*
...
```yaml
# components/cluster-config/cert-manager-config/templates/clusterissuer.yaml
...
```yaml
# components/cluster-config/ingress-controller/templates/ingress-controller.yaml
...
```yaml
# components/cluster-config/api-server/templates/api-server.yaml
...
Next: [storage network](/blog/homelab-day2/storage-network) — NMState NNCPs for the 10GbE Mellanox ports.
