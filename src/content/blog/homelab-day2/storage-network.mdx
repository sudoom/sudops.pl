...
I [filed an issue](https://github.com/okd-project/okderators-catalog-index/issues/45) and [sent the fix](https://github.com/okd-project/okd-operator-pipeline/pull/19) — stopping the build pipeline from injecting that ImageStream into the bundle in the first place, since merged upstream. When I set this up the bundle was still broken, though, so NMState comes from `operatorhubio-catalog` instead:
...
```yaml
# components/operators/nmstate/templates/operator.yaml
...
```yaml
# components/cluster-config/nmstate-nncp/templates/nncp.yaml
{{- range .Values.nodes }}
---
apiVersion: nmstate.io/v1
...
Cross-node ping from node4 over the storage network:

```
[core@node4 ~]$ ping -c2 192.168.10.3 && ping -c2 192.168.10.4
PING 192.168.10.3 (192.168.10.3) 56(84) bytes of data.
64 bytes from 192.168.10.3: icmp_seq=1 ttl=64 time=0.196 ms
64 bytes from 192.168.10.3: icmp_seq=2 ttl=64 time=0.081 ms

--- 192.168.10.3 ping statistics ---
2 packets transmitted, 2 received, 0% packet loss, time 1047ms
rtt min/avg/max/mdev = 0.081/0.138/0.196/0.057 ms
PING 192.168.10.4 (192.168.10.4) 56(84) bytes of data.
64 bytes from 192.168.10.4: icmp_seq=1 ttl=64 time=0.296 ms
64 bytes from 192.168.10.4: icmp_seq=2 ttl=64 time=0.233 ms

--- 192.168.10.4 ping statistics ---
2 packets transmitted, 2 received, 0% packet loss, time 1022ms
rtt min/avg/max/mdev = 0.233/0.264/0.296/0.031 ms
```

Sub-millisecond latency across the CRS317 switch fabric. All three nodes reachable on the storage network — this is the path Rook-Ceph will use for OSD replication in the next post.
...
```yaml
# bootstrap/root-app/values.yaml
nmstate-operator:
  enabled: true
